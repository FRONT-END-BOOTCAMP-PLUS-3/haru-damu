...
export class HealthRepositoryImpl implements HealthRepository {
  ...
  public async findByUserId(userId: number): Promise<Health | null> {
    const { data, error } = await this.supabase.from("healths").select("*").eq("user_id", userId).single();
  ...
  public async update(userId: number, updatedHealth: Partial<Health>): Promise<void> {
    const { error } = await this.supabase.from("healths").update(updatedHealth).eq("user_id", userId);
    if (error) throw new Error(error.message);
  }

  // 건강 데이터 삭제
  public async delete(userId: number): Promise<void> {
    const { error } = await this.supabase.from("healths").delete().eq("user_id", userId);
    if (error) throw new Error(error.message);
  }
}
